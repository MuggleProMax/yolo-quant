...
def download_models():
    ...
    models = {
        'face': ('yolo11n.pt', 'https://github.com/ultralytics/assets/releases/download/v8.3.0/yolo11n.pt'),
        'pose': ('yolo11n-pose.pt', 'https://github.com/ultralytics/assets/releases/download/v8.3.0/yolo11n-pose.pt')
    }
    
    for model_name, (filename, url) in models.items():
        model_path = os.path.join('models', filename)
        if not os.path.exists(model_path):
            print(f'正在下载{model_name}模型...')
            try:
                YOLO(url).save(model_path)
                print(f'{model_name}模型下载完成')
            except Exception as e:
                print(f'{model_name}模型下载失败: {str(e)}')
                return False
    return True
...
def initialize_models():
    """初始化所有模型"""
    try:
        face_model = YOLO(os.path.join('models', 'yolo11n.pt'))
...
def detect_all():
    ...
    face_model, pose_model = initialize_models()
    if None in (face_model, pose_model):
        return
    ...
    while cap.isOpened():
        ...
        if success:
            # 人脸检测
            face_results = face_model(frame, classes=[0])  # 只检测人脸
            ...
            # 行人检测
            person_results = face_model(frame, classes=[0])  # 使用同一个模型检测人
            for r in person_results:
                boxes = r.boxes
                for box in boxes:
                    if int(box.cls[0]) == 0:  # 人的类别
                        x1, y1, x2, y2 = map(int, box.xyxy[0])
                        conf = float(box.conf)
